...
const getStandardAPIStructureJson = function getStandardAPIStructureJson (request, depth) {
  ...
  queryStringJson[func.PARAM_REQUEST_PATH] = requestPath
...
const getPaginationJson = function getPaginationJson (offset, limit) {
  let pagination = {}
  pagination[func.PARAM_OFFSET] = offset
  pagination[func.PARAM_LIMIT] = limit
  return pagination
}
...
const getFilterJsonForContainsOp = function getFilterJsonForContainsOp (array, op, values) {
  let query = {}
  query[func.PARAM_ARRAY] = array
  query[func.PARAM_CONTAINS] = op
  query[func.PARAM_VALUES] = values
  return query
}
const getFilterJsonForElemMatchOp = function getFilterJsonForElemMatchOp (array, queryJson) {
  ...
  let query = {}
  query[func.PARAM_ARRAY] = array
  query[func.PARAM_ELEM_MATCH] = queryJson
...
const getStandardStructureForDBOp = function getStandardStructureForDBOp (requestRoutePath, pagination, filter, projection, sort) {
  ...
  if (filter) {
    standardApiStructure[func.PARAM_FILTER] = filter
  }
  if (projection) {
    standardApiStructure[func.PARAM_FIELDS] = projection
  }
  if (sort) {
    standardApiStructure[func.PARAM_SORT] = sort
  }
  if (requestRoutePath) {
    standardApiStructure[func.PARAM_REQUEST_PATH] = requestRoutePath
  }
